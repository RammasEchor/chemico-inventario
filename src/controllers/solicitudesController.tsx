...
function useSolicitudesController(userKey: APIStringArg) {
  const getNextFolioQuery = useQuery({
    ...
    queryFn: async () => {
      const response = await fetch(
        rootUrl + process.env.REACT_APP_BACKEND_GET_NEXT_SALIDA
      );
  ...
  const postMasterDetalleSalidaMutation = useMutation({
    mutationFn: (detalleSalida: {
      folio: APIStringArg;
      userKey: APIStringArg;
      descripcion: APIStringArg;
    }) => {
      const endpoint =
        rootUrl + process.env.REACT_APP_BACKEND_POST_MASTER_PRODUCTS_SALIDAS;
      return postFetch(endpoint, detalleSalida);
    },
    onSuccess: (res) => {
      queryClient.invalidateQueries({ queryKey: ["getProductsSalida"] });
      mutationOnSuccessReload(res);
    },
  ...
  const postDetalleSalidaMutation = useMutation({
    mutationFn: (productos: CardProduct[]) => {
      const endpoint =
        rootUrl + process.env.REACT_APP_BACKEND_POST_PRODUCTS_SALIDAS;
