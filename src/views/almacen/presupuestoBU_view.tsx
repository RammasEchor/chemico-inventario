...
function PresupuestoBUView() {
  ...
  const getAlmacenesQuery = useGetAlmacen(userKey as string);
  ...
  function redIfNull(toCheck: string | undefined) {
    if (!toCheck) return "has-text-danger";
  ...
  return (
    <div className="box">
      ...
      <Tabla cols={["Almacén", "Planta", "Presupuesto BU", "Acción"]}>
      ...
      <Modal
        key={almacenToModify.id}
        showModal={showModifyAlmacenModal}
